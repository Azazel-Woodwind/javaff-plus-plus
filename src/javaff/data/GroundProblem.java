...
public class GroundProblem implements Cloneable {
     ...
     * include
     * all propositions which could exist, but do not appear in the init or goal
     * conditions, and are not preconditions
    ...
    /**
     * Checks whether the specified goals are part of the known set of reachable
    ...
    /**
     * This method extracts all individual facts which can exist in the domain by
     * decompiling out all variants of a Fact (ADL, Equals etc). Anything which
     ...
     * in the planning graph. If a new type is introduced into the hierarchy, this
     * will
    ...
    protected static Collection<Fact> decompileFact(Fact f, final Set<Fact> staticFacts) {
        ...
        } else if (f instanceof SingleLiteral || f instanceof Function || f instanceof BinaryComparator
                || f instanceof ResourceOperator) {
            decompiled.add(f);
        } else if (f instanceof Not) {
            // Nots are a special case, and a very annoying one at that. The facts which are
            // held inside the original Not, must themselves
    ...
    /**
     * Performs reachability analysis on this groun problem by constructing a stable
     * RPG. Any facts
     * or actions which have not been achieved by the final layer of the RPG are not
     * reachable and
     * will be removed from this problem's actions and reachable facts. Calling this
     * will construct a STRIPSState
     * via {@link #getSTRIPSInitialState()}.
     * 
     * @throws UnreachableGoalException Thrown if a subset of the goal is not
     *                                  present in the final RPG layer.
     * @see #getActions()
     * @see #getReachableFacts()
     */
    ...
    /**
     * This overloaded method allows the developer to decide whether unreachable
     * goals should cause
     * an exception to be thrown. This exists purely because it is of relevance to
     * work in goal recognition.
     * 
    ...
    public void filterReachableFacts(boolean ignoreUnreachableGoal) throws UnreachableGoalException {
        ...
        if (ignoreUnreachableGoal == false) {
            if (unmetGoals.isEmpty() == false)
                throw new UnreachableGoalException(unmetGoals,
                        "Goal is not reachable through RPG reachability analysis");
        }
        ...
        System.out.println(init.getRPG().size());
        for (int i = 0; i < init.getRPG().size() - 1; i++) // TODO is -1 correct? I don't think it should make a
                                                           // difference -- layer N will be empty
        {
    ...
    /**
     * By default, GroundProblems can contain ADL actions and predicates. Calling
     * this will
     * convert the problem into a STRIPS-only form, by removing ADL from actions and
     * replacing them with
     * STRIPS-equivalent actions.
     * 
     * @param ground
     * @return The number of STRIPS actions which would have been generated,
     *         including unreachable actions
    ...
    public int decompileADL() {
        ...
        // FIXME this is pretty badly written, and incomplete. Should be recursive --
        // too many assumptions on format of data
        // keep a queue of potentially-ADL actions. Add partially compiled out actions
        // to it. When
        // no ADL constructs exist in the PCs (actions unsupported for now), it can be
        // added to the set of
        // legal actions.
        ...
        out: while (queue.isEmpty() == false) {
            ...
            if (a instanceof InstantAction) {
                for (Fact pc : a.getPreconditions()) {
                    if (this.decompileInstantAction(pc, (InstantAction) a, this.staticFacts, queue) == true) // if the
                                                                                                             // action
                                                                                                             // needed
                                                                                                             // changed
                                                                                                             // and
                ...
                // FIXME ADL in effects cannot be used unless decompileInstanceAction is
                // modified
                // with a parameter flag to indicate whether the Fact passed in is a PC or
                // Effect
                // for (Fact add : a.getAddPropositions())
                // {
                // if (this.decompileInstantAction(add, (InstantAction) a, queue) == true) //if
                // the action needed changed and requires to be reparsed
                // {
                // continue out;
                // }
                // }
                ...
                // for (Not del : a.getDeletePropositions())
                // {
                // if (this.decompileInstantAction(del, (InstantAction) a, queue) == true) //if
                // the action needed changed and requires to be reparsed
                // {
                // continue out;
                // }
            ...
            // finally, filter out any unreachable actions which have a static precondition
            // that is not in the initial state.
    ...
    /**
     * Removes all ADL code from the specified action. This is a recursive process,
     * with any modified actions being added
     * to the queue. Once an action is completely ADL-free, the method returns
     * false, indicating that the action is available
    ...
    protected boolean decompileInstantAction(Fact pc, InstantAction a, Set<Fact> staticFacts, Queue<Action> queue) {
        if (pc instanceof SingleLiteral) // if a simple proposition, then just return
        {
            ...
            for (Fact f : ((CompoundLiteral) pc).getCompoundFacts()) {
                ...
                // if false, then no modification to the action was required, so continue on as
                // normal
            }
