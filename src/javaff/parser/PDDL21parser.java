...
public class PDDL21parser implements PDDL21parserConstants {
    ...
    public static UngroundProblem parseFiles(java.io.Reader domainReader, java.io.Reader problemReader) {
        ...
        if (suc)
            probsuc = parseProblem(problemReader);
        if (probsuc) {
            // ADL quantifiers (Exists and ForAll) need to have their quantified variables
            // set up after the
            // problem file has been parsed. The problem file cannot be swapped to be parsed
            // first because
            // constants present in the domain file are needed for grounding the initial and
            // goal states.
    ...
    private static boolean parseDomain(java.io.Reader reader) {
        ...
        // return req;
        if (req)
            System.out.println("Parsed Domain successfully");
        else
            System.out.println("Parsing of Domain failed");
        return req;
    }
    ...
    private static boolean parseProblem(java.io.Reader reader) {
        ...
        // if (probsuc) System.out.println("Parsed Problem file "+pFile+"
        // successfully");
        // else System.out.println("Parsing Problem file "+pFile+" failed");
        // return probsuc;
        if (probsuc)
            System.out.println("Parsed Problem successfully");
        else
            System.out.println("Parsing Problem failed");
        return probsuc;
    }
    ...
    final public boolean parseRequireDef() throws ParseException {
        ...
        label_2: while (true) {
            ...
            if (jj_2_8(20)) {
                ...
                System.out.println("Unsupported requirement tag: " + t.toString().toLowerCase());
                {
                    if (true)
                        return false;
                }
                ...
                System.out.println("PDDL 1.7 tags not supported: " + t.toString().toLowerCase());
                {
                    if (true)
                        return false;
                }
    ...
    final public TypedList parseTypedList() throws ParseException {
        ...
        label_7: while (true) {
            ...
            type = new SimpleType("object"); // may be no explicit type declaration, so assume its just any type -- will
                                             // get overwritten if there is a type
    ...
    // boolean ground is whether it is ground or not (i.e. in an action or in the
    // initial state)
    final public Fact parseGD(Map m, boolean ground) throws ParseException {
        Literal p; // may be single or compound... should really fix one day to be in separate
                   // methods
        SingleLiteral s;
        Fact c, a, b;
        TypedList vars;
    ...
    final public Function parseFExp(Map varMap) throws ParseException {
        ...
        if (jj_2_61(20)) {
            ...
            {
                if (true)
                    return new NumberFunction(Double.parseDouble(t1.toString().toLowerCase()));
            }
        } else if (jj_2_62(20)) {
            jj_consume_token(OPEN);
    ...
    // * Parse Duration
    // **********************************
    final public DurationConstraint parseDurationConstraint(UngroundDurativeAction da, Map varMap)
    ...
    final public SimpleDurationConstraint parseSimpleDurationConstraint(UngroundDurativeAction da, Map varMap)
            throws ParseException {
        int t;
        Function f;
        jj_consume_token(OPEN);
    ...
    final public void parseObjectDeclaration() throws ParseException {
        ...
        while (it.hasNext()) {
            ...
            // problems arise if there is an object which has the same name as a constant --
            // actions
            // and facts may not have the same type information for the "same" literal. In
            // this situation
            // constants take precedence as they may be have already been parsed in the
            // Domain file (as
            // a parameter in an action schema). The object is ignored, regardless of
            // whether the types
            // are different. This is clearly not perfect, but I have been trying to find
            // out the source
            // of a grounding bug for several hours now- and I don't care about design
            // anymore.
            if (UP.constantMap.containsKey(str)) {
                // these 3 lines will remove the CONSTANT instead -- but there is no check to
                // see
