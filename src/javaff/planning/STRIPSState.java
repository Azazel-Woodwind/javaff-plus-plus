...
public class STRIPSState extends State implements Cloneable {
        ...
        if (f instanceof Proposition || TrueCondition.isSimpleTrue(f) == 1) {
            ...
            if (((Not) f).getLiteral() instanceof Not == true) {
                ...
                throw new IllegalArgumentException(
                        "STRIPSState only supports Propositions and Nots whose literal is also a Proposition");
            else
    ...
    public State apply(Action a) {
        // old code -- works but is inefficient as RPg is cloned then immediately
        // destroyed and recomputed by successor state
        // STRIPSState succ = (STRIPSState) this.clone(); //VERY slow because RPG is
        // cloned too

        // create a successor state, which is the same as this one, but has no RPG. The
        // positive and negative facts must be
        // copied to new Sets because application of actions will add/remove facts from
        // these. If a new set is not constructed
    ...
    public boolean isTrue(Fact p) {
        // 23/8/11 - all illegal statics should have been removed by now, so if a fact
        // has somehow
        // made it to here (probably embedded in a Quantified literal), then it must be
        // legal and
        // therefore true.
        ...
        // 20/5/14 -- commented out static check as ADL decompilation is causing static
        // facts which are
        // not true in the initial state to be generated, which leads to untrue facts
        // being classed as true
        // in every state.
        if (p.isStatic())
            return true;
