...
public class PlanningGraph {
    ...
    protected Set<PGAction> negativePCActions = new HashSet<PGAction>(); // quick lookup for actions which only have
                                                                         // negative PCs -- better than looking through
                                                                         // all actions at every layer
    ...
    // /**
    // * Initialise this planning graph based upon another. Note that this does not
    // actually construct
    // * the PG, see clone() for that functionality. Instead, it is intended for PGs
    // which have the same
    // * resources (actions, proposition etc), so that the internals of the PG do
    // not need to be set
    // * up every time.
    // *
    // * @param existingGraph
    ...
    /**
     * Populate a new plan graph from an existing plan graph. Note that the data
     * within the existing graph
    ...
    /**
     * The creation of a Planning Graph is an extremely costly process in both time
     * and resources. This is
     * primarily due to the need to recreate information used during the
     * construction process, but for most problems
     * this information will never change between instances of PG. Therefore, this
     * method creates a new PG while
     * retaining all this information which would otherwise be destroyed and
     * recreated (such as mutex relationships
     * etc).
     * 
     * @return An empty PlanningGraph, which has already had its preprocessing
     *         information computed/copied from
     *         this PG.
     */
    ...
    /**
     * Construct a {@link TotalOrderPlan} for the Planning graph. This method is
     * called when a Plan must be returned, and acts as a hotspot for overriding
     * what a plan
     * graph classes as a "plan".
     * 
     * @param plan A collection of collections. In this class (@{link
     *             {@link PlanningGraph}), the first collection is a list, and the
     *             second collection is a series
     *             of individual collections containing a single action in each.
     *             This is really a side-effect of having to use the RPG elsewhere
     *             in JavaFF, and the introduction of
    ...
    protected List<PGFact> filterFactLayer(List<PGFact> facts) {
        // FIXME in the same way that action which have only negative preconditions are
        // cached, so should all which have at least one negative PC
        // because this loop is extremely inefficient

        ArrayList<PGFact> set = new ArrayList<PlanningGraph.PGFact>(facts);

        // this for loop is necessary for negative preconditions. Essentially, it checks
        // if an
        // action which is otherwise scheduled to be checked at this layer (caused by it
        // having
        // at-least one positive precondition) has a negative precondition. If it does,
        // and the
        // previous layer does not explicitly have this negative fact, we increment its
        // counter
        // by 1, which is us effectively saying "this negative precondition is
        // implicitly true".
        // Without this increment, the requirement of having N actions achieve the N
        // preconditions
        // will never be met in getAvailableActions(), so the action may never be
        // applicable or
    ...
    protected void setLinks() {
        for (PGAction pga : this.actions) {
            // //special case for actions which have no preconditions, or whose
            // preconditions
            ...
            for (Not p : dels) {
                // now that negative preconditions are allowed, we have to say that deleting
                // facts
                ...
                // say that the actual deleted fact is deleted by this action, and that this
                // action
    ...
    protected ArrayList<PGAction> createFactLayer(List<PGFact> trueFacts, int pLayer) {
        ...
        // //first, we add in all actions which have no preconditions and therefore are
        // always applicable.
        // //this has to be done separately because the loop below assumes that there
        // will actually be at-least
        // //one fact true in the current relaxed state. If there are none, and the
        // first action in every plan has
         ...
         * Yet another hack to enable negative preconditions in a PG. Having checked for
         * actions which are
         * activated using the positive literals in the current state, we need to check
         * for actions which are
         * activated using *negative* preconditions. This is a subtle bug, as actions
         * which contain at-least
         * one positive precondition mask the problem at hand. However, if only negative
         * preconditions are
         * present in the action spec, then the action can only be activated by the
         * addition, and subsequent
         * deletion of the required Nots -- when in reality they should be activatable
         * right from the initial state
         * (assuming none of the Nots are true in this state -- the normal code would
         * work if Nots were present in the
         * initial state, but this is unlikely due to the sheer number of
         * possibilities).
         * I freely admit this is not a good way to do this, but the existing
         * infrastructure of JavaFF is not geared towards
         * non-STRIPS usage, so any solution is better than no solution (pun explicitly
         * and unashamedly intended).
         */
        ...
        // loop through only the actions which we know have ONLY negative preconditions
    ...
    // this is how PG actions were being chosen for quite some time (non vanilla
    // JavaFF) -- it is wrong.
    // The bug is in checking to see if an actionis applicable in the current state.
    // the problem is this current
    // state is formed in an RPG style (probalby a cut-paste job at some point),
    // where all facts are true in a
    // single state, which makes more actions applicable than should be. Current
    // version of
    ...
    // a.setCounter(a.getCounter() + 1);
    // a.setDifficulty(a.getDifficulty() + pLayer);
    // if (a instanceof PGNoOp || a.getAction().isApplicable(hackState)) //FIXME
    // this breaks non-STRIPS planning
    // {
    // a.setLayer(pLayer);
    // actionSet.add(a);
    // level_off = false;
    ...
    public List<PGAction> searchPlan(Set<PGFact> goalSet, int l) {
        ...
        // if a solution (valid solution to the PCs produced by each non mutex set of
        // parallel actions)
     ...
     * @param layer
     * @return A list of non-mutex sets of actions which can be applied at this
     *         layer
     */
    public List<Set<PGAction>> searchLevel(Set<PGFact> goalSet, int layer) {
        ...
        newGoalSet.remove(g); // pop the first thing off the queue and deal with the others in a recursive
                              // call?
